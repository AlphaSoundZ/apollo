...
function _push() {
  ...
  /*
  $Usercard_EZ = Ist die Usercard in der DB
  $User_E = Ist der User in der DB
  */
  $stmt = $pdo->prepare("SELECT * FROM rfid_devices WHERE rfid_code = ?");
...
function message($messageID) {
  ...
  switch ($messageID) {
  case 0:
      $data['message'] = $data['message']."User + Usercard wird erstellt. ";
      $data['response'] = '0';
      break;
  case 1:
      $data['message'] = $data['message']."Usercard existiert schon und wird zugewiesen. ";
      $data['response'] = '1';
      break;
  case 2:
