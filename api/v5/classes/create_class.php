...
class Create
{
        ...
        try {
            ...
            if ($th->errorInfo[1] == "1452") // check for constraint error
            {
                ...
                if (str_contains($string, "FK_user_property_class")) // class_id does not exist in property_class table
                    throw new CustomException(Response::CLASS_NOT_FOUND, "CLASS_NOT_FOUND", 400, ["class_id"]);
                if (str_contains($string, "FK_user_usercard")) // usercard_id does not exist in usercard table
                    throw new CustomException(Response::USERCARD_NOT_FOUND, "USERCARD_NOT_FOUND", 400, ["usercard_id"]);
                if (str_contains($string, "FK_user_token")) // token_id does not exist in token table
                    throw new CustomException(Response::TOKEN_NOT_FOUND, "TOKEN_NOT_FOUND", 400, ["token_id"]);
            }
            // unexpected error
            throw $th;
        }
    ...
    public static function usercard($uid, $type, $user_id = null, $allow_reassigning = false)
    {
        ...
        if ($stmt->fetch())
            throw new CustomException(Response::USERCARD_ALREADY_EXISTS, "USERCARD_ALREADY_EXISTS", 400, ["uid"]);
        ...
        if (!$stmt->fetch())
            throw new CustomException(Response::USERCARD_TYPE_NOT_FOUND, "USERCARD_TYPE_NOT_FOUND", 400, ["type"]);
        ...
        if ($user_id)
        {
            ...
            $user = $stmt->fetch();
            if (!$user)
                throw new CustomException(Response::USER_NOT_FOUND, "USER_NOT_FOUND", 400, ["user_id"]);
            
            if (isset($user["user_usercard_id"]) && $allow_reassigning == false)
                throw new CustomException(Response::USER_ALREADY_ASSIGNED, "USER_ALREADY_ASSIGNED", 400, ["user_id"]);
        }
    ...
    public static function device($uid, $type)
    {
        ...
        $stmt->execute(["uid" => $uid]);
        if ($stmt->fetch())
            throw new CustomException(Response::DEVICE_ALREADY_EXISTS, "DEVICE_ALREADY_EXISTS", 400, ["uid"]);
        ...
        $stmt->execute(["id" => $type]);
        if (!$stmt->fetch())
            throw new CustomException(Response::DEVICE_TYPE_NOT_FOUND, "DEVICE_TYPE_NOT_FOUND", 400, ["type"]);
    ...
    public static function property_class ($name, $multi_booking)
    {
        ...
        try {
            ...
            if ($th->errorInfo[1] == "1062") // check if class exists
                throw new CustomException(Response::CLASS_ALREADY_EXISTS, "CLASS_ALREADY_EXISTS", 400, ["name"]);
    ...
    public static function property_device_type ($name)
    {
        ...
        try {
            ...
            if ($th->errorInfo[1] == "1062") // check if class exists
                throw new CustomException(Response::DEVICE_TYPE_ALREADY_EXISTS, "DEVICE_TYPE_ALREADY_EXISTS", 400, ["name"]);
    ...
    public static function property_usercard_type ($name)
    {
        ...
        try {
            ...
            if ($th->errorInfo[1] == "1062") // check if class exists
                throw new CustomException(Response::USERCARD_TYPE_ALREADY_EXISTS, "USERCARD_TYPE_ALREADY_EXISTS", 400, ["name"]);
    ...
    public static function token ($username, $password, array $permissions)
    {
        ...
        if ($diff)
            throw new CustomException(Response::PERMISSION_NOT_FOUND . " (Permission id: " . implode(', ', $diff) . ")", "PERMISSION_NOT_FOUND", 400, ["permissions"]);
        if (count($permissions) !== count(array_flip($permissions))) // check for duplicates entries in $permissions array
            throw new CustomException(Response::DUPLICATE_ENTRY . ". Bitte geben Sie Permissions jeweils nur einmal an!" , "DUPLICATE_ENTRY", 400, ["permissions"]);
        ...
        try {
            ...
            if ($th->errorInfo[1] == "1062") // check if username exists
                throw new CustomException(Response::TOKEN_ALREADY_EXISTS, "TOKEN_ALREADY_EXISTS", 400, ["username"]);
